...
if($_SERVER['REQUEST_METHOD'] == 'POST'){
	if(!$headerTkn || $headerTkn != $x_token){
		...
		if(!isset($_POST['facebook_login_id'])){
			$json_return = [
				'error' => 'params',
				'msg'  => 'Parâmetros inválidos'
			];
		}else{
			$facebook_login_id = mysqli_real_escape_string($conn,$_POST['facebook_login_id']);
			if(strlen($facebook_login_id) <= 0){
				$json_return = [
					'error' => 'params.',
					'msg' => "Parâmetros inválidos"
				];
				...
				$run_cliente = mysqli_query($conn,"SELECT * FROM clientes WHERE facebook_login_id = '".$facebook_login_id."'");
						...
						if(mysqli_num_rows($run_cliente_email) <= 0){
							...
							if($cliente['imagem'] == '' && isset($_POST['imagem']) && $_POST['imagem'] != ''){
								    ...
								    (preg_match($pattern, $header[0], $match) !== false))
								{
								    ...
								    if(in_array($ext,['png','jpg','jpeg','gif','jiff'])){
								    	$newname = md5(uniqid().date('Y-m-d H:i:s')).".".($ext == 'jiff' ? 'jpg':$ext);
							...
							mysqli_query($conn,"UPDATE clientes SET facebook_login_id = '".$facebook_login_id."' WHERE id_cliente = '".$cliente['id_cliente']."'");
							$json_return = [
								'error' => false,
								'msg' => "Encontrado",
								'data' => [
									'id_cliente' => $cliente['id_cliente'],
									'google_login_id' => $cliente['google_login_id'],
