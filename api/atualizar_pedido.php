			...
			if(!is_numeric($id_pedido)){
				...
				if(mysqli_num_rows($run_pedido) != 1){
					...
					if(isset($_POST['qrcode']) && !empty($_POST['qrcode'])){
						$dados['qrcode'] = $_POST['qrcode'];
					}
					if(isset($_POST['error']) && !empty($_POST['error'])){
						if(is_array($_POST['error'])){
							try{
								$toJson = json_encode($_POST['error']);
							}catch(Exception $e){
								$toJson = json_encode(array_map('encodeUtf8',$_POST['error']));
							}
							error_log('ERROR PAGARME: '.$toJson);
						}else{
							error_log('ERROR PAGARME: '.$_POST['error']);
						}
					}
